...
def resize_video_3channel(sample_dir, result_sample_dir, deep_resize, frame_resize):
    sample_list = os.listdir(sample_dir)
    sample_list = sorted(sample_list, key=lambda x: str(x.split('.')[0].split('-')[-1]))
    sample_list = sorted(sample_list, key=lambda x: len(x.split('.')[0].split('-')[-1]))
    ...
    stack = np.zeros((len(sample_list), h, w, c), np.uint8)
    for num, img in enumerate(sample_list):
        img_dir = os.path.join(sample_dir, img)
        stack[num, :, :, :] = cv2.imread(img_dir)
    print('stack.shape: ', stack.shape)

    stack_resize = np.zeros((deep_resize, h, w, c), np.uint8)
    for h_index in range(h):
        coronal_plane = stack[:, h_index, :, :]
        coronal_plane_resize = cv2.resize(coronal_plane, (w, deep_resize))
        stack_resize[:, h_index, :, :] = coronal_plane_resize
...
if __name__ == '__main__':
    ...
    # resize_video_1channel(sample_dir, result_sample_dir, deep_resize=160, frame_resize=(512,512))

    sample_dir = r'E:\B_station\14\color_png\sunset/'
    result_sample_dir = r'E:\B_station\14\color_png\sunset_resize/'
    resize_video_3channel(sample_dir, result_sample_dir, deep_resize=666, frame_resize=(500, 500))
